...
def detect_bounce_cheque(doc, method=None):
    """Mark this Journal Entry as a bounce-cheque event, and record which
    Customer it belongs to, the moment any row uses an Account flagged
    "Is Bounce Cheque Account". Runs on validate (not just on_submit) so
    it's visible immediately while still a draft.
    ...
    bounce_customer = next(
        (row.party for row in doc.get("accounts") or [] if row.party_type == "Customer" and row.party),
        None,
    )

    is_bounce = bool(has_bounce_account and bounce_customer)
    doc.custom_is_bounce_cheque_entry = 1 if is_bounce else 0
    doc.custom_bounce_customer = bounce_customer if is_bounce else None
